...
export function fileOrder(state, action) {
  switch(action.type) {
    case 'MOVE_FILE_UP':
      var originalIndex = state.indexOf(action.id);
      if (originalIndex > 0) {
        let newState = [...state];
        newState[originalIndex] = newState[originalIndex - 1];
        newState[originalIndex - 1] = action.id;
        return newState;
      }
    case 'MOVE_FILE_DOWN':
      var originalIndex = state.indexOf(action.id);
  ...
  switch(action.type) {
    ...
    case 'SET_ROTATION_FOR_SELECTED_FILE_FRAME':
      return {
        ...
        fileFrames: state.fileFrames.map(fileFrame => {
          if (fileFrame.file === stateTree.selectedFile) {
            return {...fileFrame, ...{rotation: action.value}};
          }
          return fileFrame;
        })
      };
...
export function animations(state = defaultState.entities.animations, action, stateTree) {
  switch(action.type) {
    ...
    case 'DELETE_FRAME':
      let foundAnimation = Object.keys(state).map(key => {return state[key]}).filter(animation => { return animation.frames.indexOf(action.id) !== -1 })[0];
      return {
        ...state,
        ...{[foundAnimation.id]: {...foundAnimation, frames: foundAnimation.frames.filter(frameId => { return frameId !== action.id})}}
      };
    case 'ADD_FRAME_TO_ANIMATION':
    case 'RENAME_ANIMATION':
      return {
  ...
  switch(action.type) {
    ...
    case 'DELETE_FRAME':
      if (stateTree.selectedAnimation !== null) {
        ...
        let frameIndex = frames.indexOf(action.id);
        if (frameIndex === 0) {
          return frames[1];
        } else {
